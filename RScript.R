...
length( c( TRUE, FALSE, T, F ) )

?rep
rep( c(T,F), times = 3 )
rep( c(T,F), each = 3 )
rep( c(T,F), length.out = 7 )
...
cat( "This is the first line and \n this is second" )
...
typeof( c( T, 0L, 3.14 ) )
c( T, 0L, 3.14, "R" )
typeof( c( T, 0L, 3.14, "R" ) )

as.character( 1L )
as.logical( c( 1, 0, 3.14 ) )
as.numeric( "3.14" )
...
# All above operators are vectorized
...
set.seed( 666 )
Sample <- sample( x = 1:1000, size = 10, replace = F )
Sample
...
Sample[ Sample > 500 & Sample < 700 ]
Sample[ Sample > 2000 ]

sort( Sample )
sort( Sample, decreasing = F )
...
eduLev1 <- c( "base", "higher", "studies", 
              "higher", "base" )
eduLev2 <- factor( c( "base", "higher", "studies", 
                      "higher", "base" ), ordered = T )

eduLev1[ eduLev1 == "base" ]
eduLev1[ eduLev1 >= "higher" ]

eduLev2 <- factor( c( "base", "higher", "studies", 
                      "higher", "base" ), 
                   ordered = T, 
                   levels = c( "studies","higher","base" ) )
eduLev2
eduLev2[ eduLev2 >= "higher" ]

## Lists
vec <- c( TRUE, 3.14, "text" )
vec
typeof( vec )

list1 <- list( TRUE, 3.14, "text" )
list1
typeof( list1 )
str( list1 )

list2 <- list( 1:10, "text", mean )
list2

list3 <- list( el1 = 1:10, 11:13, 
               el3 = list( e1 = 5:7, 3:8 ) )
list3
...
lapply( X = list3[ 1:2 ], FUN = length )
Lappy_mean <- lapply( X = list3[ 1:2 ], FUN = mean )
Lappy_mean
...
x
names( x ) <- paste( "e", 1:10, sep = "_" )
x

x[ 8 ]
x[ c(  rep(F,7), T, rep(F,2) ) ]
x[ "e_8" ]

attributes( x )
str( x )

attr( x = x, "Course" ) <- "Processing massive datasets"
x
...
attr( x = x, "greater_than_5" ) <- x[ x > 5 ]
x

mean( x )

mean( attr( x = x, "greater_than_5" ) )
...
power <- function( vec, p = 2 ){
  ...
  print( mean( x ) )
  result2 <- 6
  assign( x = "global", value = 123, 
          envir = .GlobalEnv )
  Print_something <- function( r ){
    print( r )
  }
  Print_something( result2 )
  return( result1 )
}
power( y, 3 )

"C:\\Users\\p122006\\Downloads\\Exercise_4.txt"
"C:/Users/p122006/Downloads/Exercise_4.txt"

Import <- read.table(  
  file = "C:\\Users\\p122006\\Downloads\\Exercise_4.txt", 
  header = T, sep = "&" )
as.list( Import )
...
sign_vec <- function( vec ){
  res <- ifelse( vec < 0, "less than 0", 
                 ifelse( vec > 0, "greater than 0", 
                         "equals 0") )
  return( res )
}
sign_vec( -5:5 )
...
sum( 1:10 ); sum_while( 1:10 )

sum_repeat <- function( vec ){
  res <- 0
  n <- length( vec )
  i <- 1
  repeat{
    res <- res + vec[ i ]
    i <- i + 1
    if( i > n ) break # this breaks the entire loop
    # next this breaks the current iteration
  }
  return( res )
}
sum( 1:10 ); sum_repeat( 1:10 )
# repeat{} red interrupt button

sum_for1 <- function( vec ){
  res <- 0
  n <- length( vec )
  for( i in 1:n ){ # in C,C++,C# for( int i = 1; i<n; i++ )
    res <- res + vec[ i ]
  }
  return( res )
}
sum( 1:10 ); sum_for1( 1:10 )

sum_for2 <- function( vec ){ # foreach
  res <- 0
  n <- length( vec )
  for( i in vec ){ # c(10, 5, 9), i[1]=10, i[2]=5, i[2]=9
    res <- res + i
  }
  return( res )
}
sum( 1:10 ); sum_for2( 1:10 )

install.packages( "microbenchmark" )
library( "microbenchmark" )

y <- 1:1000000
microbenchmark( sum(y), sum_for1(y), sum_for2(y), 
                sum_repeat(y), sum_while(y) )
...
apply( X = x, MARGIN = 1, FUN = mean )# for rows
apply( X = x, MARGIN = 2, FUN = sum )# for columns
str( apply( X = x, MARGIN = 1, FUN = mean ) )

### data.frame
x
str(x)
cbind( x, CHAR = "test" )
str( cbind( x, CHAR = "test" ) )

x + 100
cbind( x, CHAR = "test" ) + 100

dt1 <- data.frame( key1 = 1:5, 
                   sex = c("F","M","F","M","F"),
                   height = c(1.68, 2.00, 1.50, 
                              1.80, 1.70) )
typeof( dt1 )
class( dt1 )
str( dt1 )
...
order( dt1$height )
dt1[ order( dt1$height ) ,]

dt2 <- data.frame( key2 = 1:5, 
                   sex = c("F","M","F","M","F"),
                   weight = c(100, 90, 70, 
                              50, 60) )

dtM <- merge( x = dt1, y = dt2, 
              by.x = "key1", by.y = "key2" )
# default: inner join, all.x = F all.y = F
# outer join, all.x = T all.y = T
# left join, all.x = T all.y = F
# right join, all.x = F all.y = T

aggregate( x = dtM[,c("height","weight")],
           by = list(dtM$sex.x), FUN = mean ) 
...
y <- 1:1000000
microbenchmark( builtIN_sum(y), sumR(y) )
...
cppFunction({'
String sign1( double x ){
  if( x > 0 ){ return("Positive"); }
  else if( x < 0 ){ return("Negative"); }
  else { return("Equals 0"); }
}
'})
...
sign1(-5);sign1(0);sign1(5)
sign1( -5:5 )

getwd()
list.files( getwd() )
sourceCpp( 
  file = "C:/Users/p122006/Documents/Week6/test.cpp" )
sign2(-5)
...
builtIN_sum( 1:10 ); sumC( 1:10 )

y <- 1:10000000
microbenchmark( builtIN_sum(y), sumR(y), sumC(y) )

example <- list( 1:10, 11:16 )
lapply( example, sum )

cppFunction({'
List lapplyC( List inList, Function inFunction ){
  int n = inList.size();
  List result(n);
  for( int i = 0; i < n; i++ ){
    result[i] = inFunction( inList[i] );
  }
  return( result );
}
'})
...
set.seed( 123 )
example <- list( runif(10000), rnorm( 1000 ) )
microbenchmark( lapply(example,mean), 
                lapplyC(example,mean) )
...
List1 <- list( 1:10, 11:20, 40:60 )
lapply( List1, mean )

parLapply( cluster, List1, mean )

# How to validate that all cores are busy
repeat{}
parLapply( cluster, List1, function(x){ repeat{} } )

List2 <- list( 1:1000000, 1:1000000, 
               1:1000000, 1:1000000 )
List3 <- as.list( 1:1000000 )

install.packages( "microbenchmark" )
library( "microbenchmark" )

ParPower <- function( x ){
  return( sum( x^2 + x^3 + x^4 ) )
}

lapply( List1, ParPower )

microbenchmark( Seq = lapply( List1, ParPower ), 
                Par = parLapply( cluster, List1, ParPower ) )

microbenchmark( Seq = lapply( List2, ParPower ), 
                Par = parLapply( cluster, List2, ParPower ) )

microbenchmark( Seq = lapply( List3, ParPower ), 
                Par = parLapply( cluster, List3, ParPower ) )
...
stopCluster( cluster ) # remove cluster and free the memory
cluster <- makeCluster( nCores )

parLapply( cluster, List1, ParPower )

clusterExport( cluster, varlist = c("p") )
parLapply( cluster, List1, ParPower )
...
cluster2 <- makeCluster( nCores )
registerDoSNOW( cluster2 )
foreach( i = 1:4 ) %dopar% { 
  print( i )
}
foreach( i = 1:4 ) %dopar% { 
...
Equation_model <- Volume ~ Height + Girth 
model <- lm( Equation_model, trees )
summary( model )

# 1: read.table( someData, nrows = 10 )
# 2: read.table( someData, skip = 10, nrows = 10 )
# for(){ read.table( someData, )}

dat1 <- trees[1:10,]
dat2 <- trees[11:20,]
dat3 <- trees[21:31,]

model_Big <- biglm( Equation_model, dat1 )
summary( model_Big )
model_Big <- update( model_Big, dat2 )
model_Big <- update( model_Big, dat3 )

rbind( coef( model ), coef( model_Big ) )
...
big_matrix <- matrix( runif( 10^4 * 10^4 ) , 
                      10^4, 10^4 )
object_size( big_matrix )

small_matrix <- as.big.matrix( big_matrix )
object_size( small_matrix )

str( big_matrix )
str( small_matrix )
small_matrix

small_matrix[1:5, 1:10]

install.packages("microbenchmark")
library("microbenchmark")

microbenchmark( Big = big_matrix[ big_matrix[,1] < 0.5,], 
                Small = small_matrix[ small_matrix[,1] < 0.5,] )
...
T1 <- sapply( k, function(x){ 
  as.character( sample( 1:n_obs, n_obs, T ) ) })
head(T1)

T2 <- sapply( 1:n_features, function(x){ rnorm( n_obs ) })
head(T2)
...
TT_dt <- as.data.table( TT )
str( TT_dt )

microbenchmark( Standard = TT[ TT$X2 == "2",],
                Data_table = TT_dt[ X2 == "2" ])

Filter( data = some_data, filter = "x4>0.6" )
...
XnameList <- list( "Volume ~ Height", 
                   "Volume ~ Girth", 
                   "Volume ~ Height + Girth" )
lm( Volume ~ Height, trees)
lm( XnameList[[1]], trees)

lapply(XnameList, function( Equation ){
  lm( Equation, trees)
})